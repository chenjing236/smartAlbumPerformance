...
class ComputesTest(test_base.BaseTest):
    ...
    def get_computes_request(self, token, page_size, page_index):
        url = 'http://' + self.SLB_ip + ':' + str(
            self.port) + '/v1/computes/?page_size=%s&page_index=%s' % (
    ...
    def submit_computes_with_url(self, token, fingerprint, url):
        cmd = 'curl -X POST "http://%s:%s/v1/computes/" -H "connection: close" -H "accept: application/json" -H "AuthHost: api.cloudstorage.com" -H "Token: %s" -d "fingerprint=%s&url=%s"' % \
        ...
        with open(token_file_path) as f:
            for line in f.readlines():
                ...
                t = threading.Thread(target=self.get_computes_request,
                                     args=(token, page_size, page_index))
                threads.append(t)
                i += 1
            ...
            with open(token_file_path) as f_token:
                ...
                for line_url in f_url.readlines():
                    ...
                    if not url_count % 2:
                        ...
                        t = threading.Thread(
                            target=self.submit_computes_request,
    ...
    def test_get_computes(self):
        ...
        self.multi_thread_users_get_computes(token_file_path, self.thread_count)
        self.end_time = datetime.datetime.now()
        self.out_log_pk(self.thread_count, "data/spendtime_get_compute_%s.log" % self.thread_count, self.submit_compute_spend_list)

    def test_submit_computest_tasks(self):
        ...
        self.start_time = datetime.datetime.now()
        self.multi_thread_usrs_submit_compute_tasks(token_file_path, url_file_path, self.thread_count)
        self.end_time = datetime.datetime.now()
        self.out_log_pk(self.thread_count, "data/spendtime_post_compute_%s.log" % self.thread_count, self.submit_compute_spend_list)
